...
/*   Updated: 2024/05/22 14:00:42 by lufreder         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char *av[])
{
	...
	// mlx_window = mlx_new_window(mlx_connection, mlx_ptr: game.height, game.x, so_long);
	// if (!mlx_window)
	// {
	...
	// mlx_init(); // initialise X-window connection
	// game->win = mlx_new_window(game->mlx; game->width * 32,
	//  game->height * 32, "So Long"); // create a new window
	// mlx_hook(game->win, 2, 0, key_press, game); 
	// event handling functions (key press or mouse move)
	// mlx_hook(game->win, 17, 0, close_window, game);
	// mlx_loop(game->mlx); // continuously check for events 
	// and executes associated callbacks
